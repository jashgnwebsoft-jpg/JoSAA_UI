...
import { useMotherBranchInformationQuery, useSystemBranchWiseCollegeListQuery } from '../api/hooks';
import { Button, CardContent } from '@mui/material';
import { SystemBranchWiseCollegeistResponse, SystemBranchWiseCollegeListRequest } from '../types';
...
const MotherBranchWiseCollegeListPage = () => {
  ...
  const { postModel, handlePagination, handleFiltering, handleSorting } =
    useSystemBranchWiseCollegeListStore();
  const { data: collegeOptions } = useCollegeTypeOptions();
  const { data, totalRecords, isLoading } = useSystemBranchWiseCollegeListQuery(postModel);
  const { data: motherBranchInformation } = useMotherBranchInformationQuery(systembranchID);
  const page = postModel.pageOffset + 1;
  const rowsPerPage = postModel.pageSize;

  const columns = useMemo<GridColDef<SystemBranchWiseCollegeistResponse>[]>(
    () => [
      {
  ...
  return (
    <DashboardContent>
      ...
      {/* <SimpleBreadcrumbs
        heading={motherBranchInformation?.SystemBranchProperName + ' Branch Wise Colleges'}
      ...
      <MainContent
        breadCrumbsProps={{
          heading:
            ...
            motherBranchInformation?.SystemBranchProperName + ' Branch Wise College',
          links: [{ name: 'Home' }, { name: t('Institute.Branch.MotherBranchWiseColleges.Label') }],
        }}
        sx={{ p: 0 }}
      >
